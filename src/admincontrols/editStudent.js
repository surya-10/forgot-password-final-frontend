import { useEffect, useState } from "react";
import Base from "./base";
import { useNavigate, useParams } from "react-router-dom";
import axios from "axios";



function EditStudents() {
    let navigate = useNavigate()
    let [show, setShow] = useState(false);
    let [status, setStatus] = useState("false");
    let [imageVal, setImageVal] = useState({
        photo:null
    });
    let [userData, setUserData] = useState({
        firstname: '',
        lastname: '',
        email: '',
        age: '',
        dob: '',
        college: '',
        about: '',
        number: '',
        gender: '',
        photo: null,
    });
    let { id } = useParams();
    useEffect(() => {
        findStudent(id);
    }, [id]);


    async function findStudent(id) {
        setShow(true);
        let result = await fetch(`https://forgotpassword-mbwj.onrender.com/access/student/${id}`, {
            method: "GET",
            headers: {
                "content-type": "application/json"
            }
        })
        let studentsData = await result.json();
        console.log(studentsData);
        setUserData(studentsData.data);
        setImageVal(studentsData.data.photo);
        setShow(false);
    }


    const handleChange = (e) => {
        setUserData({ ...userData, [e.target.name]: e.target.value });
    };
    async function handleImageChange(e) {
        setStatus("true");
        setImageVal({...imageVal, photo: e.target.files[0]});
        // setUserData({ ...userData, photo: e.target.files[0] });
    }

    async function handleSubmit(e) {
        
        e.preventDefault();
        setShow(true);
        const dataToSend = new FormData();
        dataToSend.append('firstname', userData.firstname);
        dataToSend.append('lastname', userData.lastname);
        dataToSend.append('email', userData.email);
        dataToSend.append("age", userData.age);
        dataToSend.append("dob", userData.dob);
        dataToSend.append("college", userData.college);
        dataToSend.append("number", userData.number);
        dataToSend.append("gender", userData.gender);
        dataToSend.append("about", userData.about);
        dataToSend.append("status", status)
        // dataToSend.append("phtot", userData.photo)

        let secondDataTosend = new FormData();

        secondDataTosend.append("photo", imageVal.photo);

        console.log(imageVal.photo);
        console.log(secondDataTosend)
        if(imageVal.photo){

        try {
            let result = await axios.post(`https://forgotpassword-mbwj.onrender.com/update/user/${id}`, secondDataTosend);
            if (result.data.status == 201) {
                alert("student data updated");
                let remaingData = await axios.post(`https://forgotpassword-mbwj.onrender.com/update/user/data/${id}`, dataToSend);
                setShow(false);
                if(remaingData.data.status===200){
                    navigate("/admin/auth/all");
                }

            }
            if (result.data.status == 404) {
                alert("student data already exist");
            }
        }
      catch (error) {
            console.error('Error submitting form:', error);
            alert('Error submitting form. Please try again.');
        }
    }
    else{
        try{
            setShow(true);
            let remaingData = await axios.post(`https://forgotpassword-mbwj.onrender.com/update/user/data/${id}`, dataToSend);
                if(remaingData.data.status===200){
                    setShow(false);
                    navigate("/admin/auth/all");
                }
        }
        catch(error){
            console.error('Error submitting form:', error);
            alert('Error submitting form. Please try again.');
        }
    }
}
    return (
        <div className="add-dtud-div">
            <Base>
                <div className="container d-flex justify-content-center align-items-center min-vh-100 flex-column mt-5">
                    <div className="d-flex align-self-center">
                        <p className="fs-6 tx-prim fw-bold head">Edit selected student data.</p>
                    </div>
                    <div className="form-div d-flex flex-column justify-content-center align-items-center p-2">

                        <div className="form1">
                            <form onSubmit={handleSubmit}>
                                <div className="form-inside">
                                    <p className="fs-5 add-title">Add New Student</p>
                                    <div className="names d-flex">

                                        <div className="f-name d-flex flex-column justify-content-start align-items-start">
                                            <label><small className="text-uppercase fw-bolder mb-2">First name</small></label>
                                            <input type="text" placeholder="First name" className="form-control"
                                                name="firstname"
                                                value={userData.firstname}
                                                onChange={handleChange} required
                                            />
                                        </div>
                                        <div className="l-name d-flex flex-column justify-content-start align-items-start">
                                            <label><small className="text-uppercase fw-bolder mb-2">Last name</small></label>
                                            <input type="text" placeholder="Last name" className="form-control"
                                                name="lastname"
                                                value={userData.lastname}
                                                required
                                                onChange={handleChange} />
                                        </div>
                                    </div>
                                    <div className="names d-flex ">

                                        <div className="f-name d-flex flex-column justify-content-start align-items-start">
                                            <label><small className="text-uppercase fw-bolder mb-2">Email</small></label>
                                            <input type="email" placeholder="email id" className="form-control"
                                                name="email"
                                                value={userData.email}
                                                required
                                                onChange={handleChange}
                                            />
                                        </div>
                                        <div className="l-name d-flex flex-column justify-content-start align-items-start">
                                            <label><small className="text-uppercase fw-bolder mb-2">Phone number</small></label>
                                            <input type="text" placeholder="phone number" className="form-control"
                                                name="number"
                                                value={userData.number}
                                                required
                                                onChange={handleChange} />
                                        </div>
                                    </div>
                                    <div className="names d-flex ">

                                        <div className="f-name d-flex flex-column justify-content-start align-items-start">
                                            <label><small className="text-uppercase fw-bolder mb-2">Age</small></label>
                                            <input type="text" placeholder="age" className="form-control"
                                                name="age"
                                                value={userData.age}
                                                required
                                                onChange={handleChange} />
                                        </div>
                                        <div className="l-name d-flex flex-column justify-content-start align-items-start">
                                            <label><small className="text-uppercase fw-bolder mb-2">Joined Date</small></label>
                                            <input type="date" placeholder="phone number" className="form-control"
                                                name="dob"
                                                value={userData.dob}
                                                required
                                                onChange={handleChange} />
                                        </div>
                                    </div>
                                    <div className="d-flex flex-column align-items-start">
                                        <label><small className="text-uppercase fw-bolder mb-2">College name</small></label>
                                        <input type="text" placeholder="college name" className="form-control"
                                            name="college"
                                            value={userData.college}
                                            required
                                            onChange={handleChange} />
                                    </div>
                                    <div className="d-flex flex-column align-items-stretch">
                                        <select name="gender" value={userData.gender} onChange={handleChange} required>
                                            <option value="">Select Gender</option>
                                            <option value="male">Male</option>
                                            <option value="female">Female</option>
                                            <option value="other">Other</option>
                                        </select>
                                    </div>
                                    <div className="form-floating">
                                        <textarea className="form-control" placeholder="Leave a comment here" id="floatingTextarea2" style={{ height: 100 + "px" }}
                                            name="about"
                                            value={userData.about}
                                            required
                                            onChange={handleChange}></textarea>
                                        <label htmlFor="floatingTextarea2">About student</label>
                                    </div>
                                    <div className="upload-img d-flex justify-content-start align-items-start flex-column">
                                        <label><small className="text-uppercase fw-bolder mb-5">Student image - Optional   size: less than 4MB</small></label>
                                        <div className="input-group mb-3 mt-2">
                                            <img src={`data:image/jpeg;base64,${userData.photo}`}
                                                alt={`Photo of ${userData.name}`}
                                                style={{ width: '30px', height: '40px' }}
                                                className="me-2" />
                                            <input type="file" className="form-control" id="inputGroupFile02" accept="image/*"
                                                name="image"

                                                onChange={handleImageChange} />
                                            <label className="input-group-text" htmlFor="inputGroupFile02">Upload</label>
                                        </div>
                                    </div>
                                    <div className="add-btn">
                                        <button className="btn bg-success text-white" type="submit">Update</button>
                                    </div>
                                </div>
                            </form>
                        </div>
                    </div>

                </div>
            </Base>
            {show && <div className="spin">
                <div className="spinner-border" role="status">
                    <span className="visually-hidden">Loading...</span>
                </div>
            </div>}
        </div>
    )
}
export default EditStudents;